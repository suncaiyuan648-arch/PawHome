<template>
	<view class="page" :class="pageState === 'publish-entry' ? 'publish-mode' : (pageState === 'roster' || pageState === 'status' ? 'roster-mode' : 'long-mode')">
		<!-- #ifndef MP-WEIXIN -->
		<image v-if="pageState === 'publish-entry'" class="yard-cats-top-reference" src="/static/figma/yard-cats-top.png" mode="scaleToFill" />
		<!-- #endif -->
		<view v-if="pageState === 'roster' || pageState === 'status'" class="roster-wrap"><PawPetRoster :variant="pageState === 'status' ? 'status' : 'yard'" @back="goBack" /></view>
		<view v-if="pageState === 'publish-entry'" class="publish-nav" :style="{ paddingTop: statusBarHeight + 'px' }">
			<view class="publish-owner-row">
				...
				<image class="publish-owner-avatar" src="/static/figma/search/yard-avatar-exact.png" mode="aspectFill"></image>
				<text class="publish-owner-name">{{ yardName }}</text><text class="publish-owner-tag">小院</text>
			</view>
			<view class="publish-title-row"><text>小院猫咪</text><view class="publish-add" @click="onAdd">添加</view></view>
		</view>
		<view v-else class="nav-wrap" :style="{ paddingTop: statusBarHeight + 'px' }">
			<view class="nav-row">
				...
				<view class="nav-title-wrap">
					...
					<view class="yard-search"><uni-icons type="search" color="#bbbbbb" :size="14" /><text>搜索宠物名字/品种等</text><view class="yard-search-btn">搜一搜</view></view>
				</view>
			</view>
		</view>
		<view v-if="pageState !== 'publish-entry'" class="category-tabs"><text class="category-tab category-tab--active">全部(23)</text><text class="category-tab">猫咪(22)</text><text class="category-tab">狗狗(1)</text><view class="category-sort"><text>智能排序</text><uni-icons type="down" color="#333333" :size="12" /></view><uni-icons type="list" color="#333333" :size="20" /></view>
		...
		<scroll-view class="main-scroll" scroll-y :show-scrollbar="false">
			<view class="scroll-inner">
				<view v-for="block in displayBlocks" :key="block.key" class="cat-card">
					<view class="cat-badge" :class="['cat-badge--' + block.key, { 'cat-badge--publish': pageState === 'publish-entry' }]"><text>{{ block.label }}</text></view>
					<view class="cat-avatars">
						<view
							v-for="(cat, i) in block.cats"
							:key="cat.id"
		...
		<view v-if="showCatDetail && selectedCat" class="detail-pop" @click.stop>
			...
			<view class="pop-inner">
				...
				<view class="pop-tags">
					<view v-for="(t, ti) in selectedCat.tags" :key="'tg-' + ti" class="pop-tag"><text>{{ t }}</text></view>
				</view>
		...
		<!-- 添加宠物类型（底部弹层） -->
		<view v-if="showAddPetSheet" class="add-mask" @click="closeAddPetSheet"></view>
		<view v-if="showAddPetSheet" class="add-sheet" @click.stop>
			<view class="add-head">
				<text class="add-title">请选择要添加的宠物类型</text>
				<view class="add-close" @click="closeAddPetSheet"><text>×</text></view>
			</view>
			<view class="add-cards">
				<view
					class="add-card"
					:class="{ 'add-card--selected': addPetKind === 'cat' }"
					@click="addPetKind = 'cat'"
				>
					<text class="add-card-txt" :class="{ 'add-card-txt--muted': addPetKind !== 'cat' }">猫猫</text>
				</view>
				<view
					class="add-card"
					:class="{ 'add-card--selected': addPetKind === 'dog' }"
					@click="addPetKind = 'dog'"
				>
					<text class="add-card-txt" :class="{ 'add-card-txt--muted': addPetKind !== 'dog' }">狗狗</text>
				</view>
			</view>
			<view class="add-footer">
				<view class="add-submit" @click="onAddPetConfirm"><text>添加进小院</text></view>
			</view>
		</view>
...
const mockCat = (id, name, extra = {}) => ({
	id,
	name,
	avatar: extra.avatar || '/static/figma/home/pet-cat.png',
	tags: extra.tags || ['中华田园猫', '男生', '已绝育', '2岁3个月'],
	desc:
		extra.desc ||
		'小黄是我见过最乖最帅最萌的小猫，饭量很大，希望可以多多投喂猫粮给它',
	foodJin: extra.foodJin != null ? extra.foodJin : 32
})

export default {
	components: { YardTagPill, PawPetRoster },
	data() {
		return {
			pageState: 'long-list',
			statusBarHeight: 20,
			...
			blocks: [
				{
					...
					label: '待领养',
					cats: [
						mockCat('p1', '奥利奥', { foodJin: 6 }),
						mockCat('p2', '呗呗', { avatar: '/static/figma/home/pet-dog.png', foodJin: 10 }),
						mockCat('p3', '呗呗', { avatar: '/static/figma/home/pet-dog.png', foodJin: 12 }),
						mockCat('p4', '呗呗', {
							avatar: '/static/figma/home/pet-dog.png',
							desc: '小黄是我见过最乖最帅最萌的小猫，饭量很大，希望可以多多投喂猫粮给它',
							foodJin: 32
						}),
						mockCat('p5', '呗呗', { avatar: '/static/figma/home/pet-dog.png', foodJin: 5 })
					]
				},
				{
					key: 'cloud',
					label: '已云养',
					cats: [mockCat('c1', '奥利奥'), mockCat('c2', '呗呗', { avatar: '/static/figma/home/pet-dog.png' })]
				},
				{
					key: 'adopted',
					label: '已领养',
					cats: [mockCat('a1', '奥利奥', { tags: ['英短', '女生', '已绝育', '1岁'], foodJin: 20 }), mockCat('a2', '呗呗', { avatar: '/static/figma/home/pet-dog.png' })]
				},
				{
					key: 'missing',
					label: '失踪',
					cats: [mockCat('m1', '奥利奥'), mockCat('m2', '呗呗', { avatar: '/static/figma/home/pet-dog.png' })]
				},
				{
					key: 'dead',
					label: '死亡',
					cats: [mockCat('d1', '奥利奥'), mockCat('d2', '呗呗', { avatar: '/static/figma/home/pet-dog.png' })]
				}
			],
			showCatDetail: false,
			selectedCat: null,
	...
	onLoad(query = {}) {
		...
		// #endif
		this.pageState = ['publish-entry','roster','status'].includes(query.state) ? query.state : 'long-list'
		// #ifdef MP-WEIXIN
		try {
...
.publish-owner-avatar { width: 32px; height: 32px; border-radius: 50%; margin-left: -4px; }
.publish-owner-name { margin-left: 9px; font-size: 14px; color: #222; }
.publish-owner-tag { margin-left: 6px; padding: 2px 6px; border-radius: 8px; background: #ffed53; font-size: 10px; color: #333; }
.publish-title-row { height: 54px; display: flex; align-items: center; justify-content: space-between; padding: 0 15px; box-sizing: border-box; }
.publish-title-row>text,.publish-title-row>.publish-add{position:relative;top:-4px}
.publish-title-row > text { font-size: 22px; line-height: 28px; font-weight: 700; color: #222; }
.publish-add { width: 50px; height: 30px; border-radius: 5px; background: #ffe000; display: flex; align-items: center; justify-content: center; font-size: 14px; color: #222; }
.nav-title-wrap {
	flex: 1;
	min-width: 0;
	display: flex;
...
.yard-search{display:flex;align-items:center;min-width:0;flex:1;height:58rpx;padding-left:16rpx;border:3rpx solid #222;border-radius:30rpx;background:#fff;color:#bbb;font-size:20rpx}.yard-search>text{overflow:hidden;flex:1;margin-left:6rpx;text-overflow:ellipsis;white-space:nowrap}.yard-search-btn{display:flex;align-items:center;align-self:stretch;padding:0 18rpx;border-radius:28rpx;background:#222;color:#fff;white-space:nowrap}.category-tabs{display:flex;align-items:center;height:64rpx;padding:0 18rpx;background:#fff;font-size:22rpx}.category-tab{padding:12rpx 24rpx;border-radius:8rpx;background:#f5f5f5}.category-tab--active{background:#ffdf00}.category-sort{margin-left:auto;margin-right:10rpx;display:flex;align-items:center;gap:2rpx}
.main-scroll {
	flex: 1;
	height: 0;
	width: 100%;
...
.cat-badge--cloud { background: #ffe60f; }
.cat-badge--cloud text { color: #111; }
.cat-badge--adopted.cat-badge--publish { background: #c9f47b; }
.cat-badge--missing {
	background: #8a8a8a;
}
...
.publish-mode .cat-card { min-height: 154px; }
.publish-mode .cat-card:first-child { min-height: 172px; }
.publish-mode .scroll-inner { padding-top: 6px; }
.publish-mode .cat-card { margin-bottom: 32px; padding-top: 33px; }
.publish-mode .cat-card:nth-child(2) { margin-bottom: 34px; }
.publish-mode .cat-avatars{padding-left:11px;padding-right:15px;justify-content:flex-start;column-gap:21.25px;box-sizing:border-box}
.publish-mode .cat-avatar-item{width:44px}
.publish-mode .cat-badge { padding-top: 2px; padding-bottom: 2px; }
.publish-mode .cat-count { position:absolute;right:12px;bottom:14px;margin-top:0; }
.long-mode .cat-card { min-height: 184px; }
.long-mode .cat-card:first-child { min-height: 200px; }
.done-btn text {
	font-size: 34rpx;
	font-weight: 700;
	color: #111;
