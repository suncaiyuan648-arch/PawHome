    ...
    <scroll-view class="extra-scroll" scroll-y :show-scrollbar="false">
      ...
      <template v-else-if="mode === 'quota-detail'">
        ...
        <view class="order-card paw-surface">
          ...
          <view><text>云养 豆豆</text><text>3天</text></view>
        </view>
      </template>
      <template v-else>
...
export default {
  ...
  data() {
    return {
      mode: 'support', recordId: '', supportList: [
  ...
  onLoad(options = {}) { const mode = String(options.mode || 'support'); this.mode = ['support', 'quota', 'quota-detail'].includes(mode) ? mode : 'support'; this.recordId = String(options.id || options.recordId || ''); this.showInsufficient = options.popup === 'insufficient' },
  methods: {
    openDetail() { uni.navigateTo({ url: `/pages/adoption/extras?mode=quota-detail${this.recordId ? `&id=${encodeURIComponent(this.recordId)}` : ''}` }) },
    openHelpedAnimals() { uni.navigateTo({ url: `/pages/meMore/helpedAnimals${this.recordId ? `?id=${encodeURIComponent(this.recordId)}` : ''}` }) }
  }
}
</script>
