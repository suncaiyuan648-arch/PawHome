<template>
	<view class="container" :class="{ 'state-scrolled': pageState === 'dynamic-scrolled' || hideTopActions }"
		@wheel.capture="onFeedWheel" @pointerdown.capture="onFeedPointerStart" @pointermove.capture="onFeedPointerMove"
		@pointerup.capture="onFeedPointerEnd" @pointercancel.capture="onFeedPointerEnd">
		<!-- #ifndef MP-WEIXIN -->
		<image class="h5-status-bar"
			:src="activeFeedTab === 'yard' ? '/static/figma/home-header-yard-exact.png' : '/static/figma/home-header-exact.png'"
		...
		<view class="container2" :class="{ hidden: hideTopActions }">
			...
			<view class="box2">
				<view class="search">
					<PawSearchBar class="home-search-bar" :readonly="true"
						:placeholder="activeFeedTab === 'yard' ? '蓝金渐层' : '小院号'" @tap="openSearchPage"
						@search="openSearchPage" />
					<view class="search-btn" @click="openLeaderboard"><text>排行榜</text></view>
				</view>
				<view class="home-announcement">
					<PawAnnouncementMarquee ref="homeAnnouncement" :items="announcementItems" :speed="82" :gap="1000"
						:poll-url="announcementPollUrl" :ws-url="announcementWsUrl" />
				</view>
			</view>
		</view>
		<view class="tab" :class="{ 'tab-scrolled': hideTopActions }">
			<view class="tab-left">
				<view v-for="tab in feedTabs" :key="tab.key" class="tab-item"
					:class="{ active: activeFeedTab === tab.key }" @click="changeFeedTab(tab.key)">{{ tab.label }}
				</view>
			</view>
			<PawPopoverMenu v-model="showSortDropdown" :items="sortOptions" :active-key="selectedSort"
				@select="selectSort">
				<template #trigger>
		...
		<scroll-view class="feed-scroll" scroll-y="true" :enable-flex="true" :bounces="false" :refresher-enabled="true"
			refresher-default-style="none" refresher-background="transparent" :refresher-triggered="refresherTriggered"
			scroll-with-animation :scroll-into-view="scrollIntoViewId" @scroll="handleFeedScroll"
			@scrolltoupper="onFeedScrollToUpper" @touchstart.capture="onFeedTouchStart"
			@touchmove.capture="onFeedTouchMove" @touchend.capture="onFeedTouchEnd"
			@touchcancel.capture="onFeedTouchEnd" @refresherpulling="onRefresherPulling"
			@refresherrefresh="onPullRefresh" @scrolltolower="onReachBottom" :upper-threshold="4" :lower-threshold="80"
			...
			<view class="feed-scroll-inner">
				...
				<view v-else-if="activeFeedTab === 'yard'" class="home-yard-list">
					<YardSummaryCard v-for="yard in yardCards" :key="yard.id" class="home-yard-card"
						:yard="yardModel(yard)" variant="list" @click="goYardDetail" />
				</view>
				<view v-else class="paw-list" :class="{ 'tab-switching': isTabSwitching }">
					<view class="paw-column" v-for="(column, columnIndex) in feedColumns"
						:key="'feed-column-' + columnIndex">
						<FeedCard v-for="entry in column" :key="'fc-' + entry.index" :item="entry.item"
							@click="goDetail" @user-click="openAuthorProfile" @like="toggleFeedCardLike(entry.index)" />
					</view>
				</view>
				<view v-if="!isDynamicEmpty && activeFeedTab !== 'yard' && (isLoadingMore || !hasMore)"
					id="qa-home-load-more" class="load-more-indicator">
