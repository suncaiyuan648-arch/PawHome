<template>
	<PawFlowResult v-if="!showFeeding" title="发布成功" :body="successBody" button-text="查看动态" @back="goBack"
		@action="viewFeed" />
	<view v-else class="page">
		...
		<PawSuccessOverlay :visible="showFeeding" mode="feeding" title="投喂成功" :body="successBody" button-text="查看投喂订单"
			@close="showFeeding = false" @action="viewFeed" />
	</view>
</template>
...
const SUCCESS_BODY = '饿了就睡 醒了就找\n日子浑浑噩噩\n谢谢你给我一口粮\n也给我一点盼头'
const PUBLISHED_DYNAMIC_ID = 'published-dynamic-1'
...
export default {
	...
	data() {
		return {
			...
			publishedDynamicId: PUBLISHED_DYNAMIC_ID
		}
	},
	onLoad(options = {}) {
	...
	methods: {
		...
		viewFeed() {
			uni.navigateTo({
				url: `/pages/dynamicDetail/index?yardId=1&dynamicId=${encodeURIComponent(this.publishedDynamicId)}`
			})
		}
	}
}
