	...
	<view class="profile-page">
		...
		<view class="hero" :style="{ paddingTop: statusBarHeight + 'px' }">
			...
			<view class="hero-inner">
					...
					<view class="stats-follow">
						...
						<view
							class="follow-pill"
							:class="{ 'follow-pill--followed': followed }"
							@click.stop="onFollowTap"
						>
			...
			<view class="panel-wrap">
				<view v-if="activeTab === 'review'" class="review-panel">
					...
					<view v-for="item in reviewList" :key="item.id" class="review-row">
						...
						<view class="review-like"><uni-icons type="hand-up-filled" color="#ff334d" :size="16"></uni-icons><text>{{ item.likes }}</text></view>
					</view>
				</view>
						...
						<view class="feed-foot">
							...
							<view class="feed-foot-like" @click.stop="toggleProfileFeedLike(idx)">
								<uni-icons
									...
									:color="item.liked ? '#ff2741' : '#b8b7b3'"
									:size="14"
								></uni-icons>
					...
					>
						...
						<scroll-view class="adopt-gallery-scroll" scroll-x :show-scrollbar="false" :enable-flex="true">
							<view class="adopt-gallery-row">
								<view
									v-for="(c, ci) in row.cats"
									:key="ci"
									class="adopt-cat-cell"
								>
					...
					>
						...
						<view class="donate-right">
							<view class="donate-badge-wrap">
								<view
									class="donate-badge"
									:class="'donate-badge--' + row.topBadge.kind"
								>
									<text class="donate-badge-txt">{{ row.topBadge.text }}</text>
								</view>
								<view
									v-if="row.topBadge.notify != null"
									class="donate-notify"
								>
							...
							<text
								v-if="row.progressText"
								class="donate-progress"
				...
				<view
					v-else-if="activeTab === 'yard' || activeTab === 'joined'"
					class="joined-list"
				>
					<view
						v-for="(item, ji) in joinedLikeList"
						:key="(activeTab === 'yard' ? 'yd-' : 'jn-') + ji"
						class="joined-card"
					>
		...
		<scroll-view v-else class="profile-timeline-scroll" scroll-y :show-scrollbar="false" :bounces="false">
			<view class="profile-timeline-panel">
				<view class="timeline-heading"><text>评价</text><view class="timeline-heading-line"></view></view>
				<view v-for="item in profileTimeline" :key="item.day + item.month" class="profile-timeline-row">
					<view class="timeline-axis"><view class="timeline-dot"></view><view class="timeline-line"></view></view>
					<view class="timeline-content">
						<view class="timeline-date"><text class="timeline-day">{{ item.day }}</text><text v-if="item.month" class="timeline-month">{{ item.month }}</text><text class="timeline-year">2026年</text></view>
						<text class="timeline-action">{{ item.action }}</text>
						<view v-if="item.copy" class="timeline-copy-row"><image class="timeline-avatar" src="/static/figma/profile/timeline-1.png" mode="aspectFill"></image><text>{{ item.copy }}</text></view>
						<view v-if="item.images" class="timeline-gallery"><image v-for="src in item.images" :key="src" :src="src" mode="aspectFill"></image></view>
		...
		<!-- 点击头部「投粮」统计弹出 -->
		<view
			v-if="showDonateSummaryPopup"
			class="donate-sum-mask"
			@click.self="closeDonateSummary"
		>
		...
		<view
			...
			class="unfollow-mask"
			@click.self="closeUnfollowConfirm"
		>
...
<style scoped>
	...
	.more-sheet-panel {
		...
		box-sizing: border-box;
		animation: more-sheet-up 0.22s ease-out;
	}
	...
	.more-sheet-item--danger .more-sheet-txt {
		color: #ff2741;
		font-weight: 500;
	}

	.more-sheet-item--strong .more-sheet-txt {
		color: #111111;
		font-weight: 500;
	}

	.more-sheet-item--muted .more-sheet-txt {
		color: #999999;
		font-weight: 400;
	}
	...
	.review-panel { min-height: 780rpx; padding: 18rpx 32rpx 40rpx; box-sizing: border-box; background: #fff; }
	.review-filters { display: flex; gap: 18rpx; margin-bottom: 28rpx; position: relative; top: -10rpx; }
	.review-filter { display: flex; align-items: center; justify-content: center; min-width: 92rpx; height: 48rpx; padding: 0 18rpx; border-radius: 10rpx; background: #f5f5f5; color: #666; font-size: 24rpx; }
	.review-filter--active { background: #ffdf00; color: #111; }
	.review-row { display: flex; align-items: flex-start; }
	.review-avatar { flex: 0 0 auto; width: 64rpx; height: 64rpx; border-radius: 50%; }
	.review-main { min-width: 0; flex: 1; margin-left: 16rpx; }
	.review-name-line { display: flex; align-items: center; }
	.review-name { color: #444; font-size: 24rpx; }
	.review-copy { display: block; margin-top: 6rpx; color: #333; font-size: 22rpx; line-height: 28rpx; }
	.review-meta { display: block; margin-top: 14rpx; color: #999; font-size: 20rpx; }
	.review-like { display: flex; gap: 8rpx; align-items: center; padding-top: 64rpx; color: #666; font-size: 22rpx; }

	.profile-timeline-scroll { flex: 1; width: 100%; height: 0; min-height: 0; margin-top: -40rpx; position: relative; z-index: 5; border-radius: 24rpx 24rpx 0 0; background:#fff; }
	.profile-timeline-panel { min-height: 100%; padding: 22rpx 30rpx 44rpx; box-sizing: border-box; background:#fff; }
	.timeline-heading { position: relative; height: 58rpx; text-align:center; color:#999; font-size:28rpx; line-height:48rpx; }
	.timeline-heading-line { position:absolute; left:0; bottom:0; width:56rpx; height:6rpx; background:#ffe20a; border-radius:4rpx; }
	.profile-timeline-row { position:relative; display:flex; min-height:250rpx; }
	.timeline-axis { position:relative; flex:0 0 28rpx; }
	.timeline-dot { position:absolute; left:0; top:21rpx; width:14rpx; height:14rpx; border-radius:50%; border:2rpx solid #fff; background:#ffe20a; box-shadow:0 0 0 1rpx #ddd; z-index:2; box-sizing:border-box; }
	.timeline-line { position:absolute; left:6rpx; top:31rpx; bottom:-18rpx; width:1rpx; background:#e8e8e8; }
	...
	.timeline-avatar { flex:0 0 auto; width:66rpx; height:66rpx; margin-right:18rpx; border-radius:12rpx; }
	.timeline-gallery { display:flex; gap:8rpx; margin-top:20rpx; }
	.timeline-gallery image { width:calc((100% - 16rpx)/3); height:150rpx; border-radius:4rpx; }
	.timeline-wide { display:block; width:100%; height:276rpx; margin-top:24rpx; border-radius:10rpx; }
</style>
