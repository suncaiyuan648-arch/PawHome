	...
	<view class="feed-page">
		...
		<view class="nav-wrap" :style="{ paddingTop: statusBarHeight + 'px' }">
			<view class="nav-row">
				...
				<PawSearchBar v-if="ownerMode" class="owner-search" v-model="ownerKeyword" placeholder="搜索宠物名字/品种等"
					@search="onOwnerSearch" />
				<view class="nav-side nav-right" :style="{ width: menuRightWidth + 'px' }"></view>
			</view>
			<view v-if="!ownerMode" class="page-title-line"><text>小院投粮</text>
				<view v-if="navBadge > 0" class="nav-title-badge"><text>{{ navBadge }}</text></view>
			</view>
			<view v-else class="owner-filters">
				<view>列表展示⌄</view>
				<view>智能排序⌄</view>
			</view>
		</view>
		...
		<scroll-view class="main-scroll" scroll-y :show-scrollbar="false" :bounces="false">
			<view class="list-pad">
				<view v-for="item in list" :key="item.id" class="feed-card" @click="openDetail(item)">
					<view class="card-avatar-wrap" @click.stop="openYardRow(item)">
						<image class="card-yard-icon" src="/static/figma/feeding/yard-feed-avatar.png"
					...
					<view class="card-mid">
						...
						<view v-if="ownerMode" class="owner-order-state"
							:class="'owner-order-state--' + item.orderTone"><text class="owner-state-pill">{{
								item.orderState }}</text><text>{{ item.orderCopy }}</text></view>
					</view>
					<view v-if="!ownerMode" class="card-right">
						<text class="top-status" :class="{
...
export default {
	components: { LevelCapsule, PawSearchBar },
	data() {
		return {
			statusBarHeight: 20,
			menuRightWidth: 87,
	...
	methods: {
		onOwnerSearch(value) {
			this.ownerKeyword = String(value || '').trim();
		},
		goBack() {
...
.page-title-line .nav-title-badge {
	margin-left: 2rpx;
	margin-top: -18rpx;
}
...
.owner-order-state--blue {
	color: #1786ff;
}

.owner-order-state--red {
	color: #ff3b45;
}

.owner-order-state--green {
	color: #08bd68;
}

.owner-order-state--orange {
	color: #ff8a24;
}
