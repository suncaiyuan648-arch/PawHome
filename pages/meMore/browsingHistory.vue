<template>
	<view class="hist-page">
		...
		<view class="tabs">
			<view class="tab-item" :class="{ 'tab-item--active': activeTab === 'feed' }" @click="activeTab = 'feed'">
				<text class="tab-text">动态</text>
			</view>
			<view class="tab-item" :class="{ 'tab-item--active': activeTab === 'yard' }" @click="activeTab = 'yard'">
				<text class="tab-text">小院</text>
			</view>
		</view>

		<scroll-view class="main-scroll" :class="{ 'main-scroll--white': isCurrentEmpty }" scroll-y
			:show-scrollbar="false" :bounces="false">
				...
				<view v-for="(item, idx) in feedList" :key="'f-' + idx" class="hist-card" @click="onFeedTap(item)">
					...
					<view class="hist-card-foot">
						<view class="hist-foot-left" @click.stop="openUserFromItem(item)">
							<PawAvatar class="hist-avatar" :src="item.userAvatar" fallback="/static/user.png"
								size="36rpx" />
			...
			<view v-else-if="activeTab === 'yard'" class="yard-list">
				<view v-for="(item, idx) in yardList" :key="'y-' + idx" class="yard-card" @click="onYardTap(item)">
					<view class="yard-header">
						<PawAvatar class="yard-avatar" :src="item.userAvatar" fallback="/static/user.png" :size="48"
							:clickable="true" @click.stop="openUserFromItem(item)" />
						<view class="yard-header-mid">
							...
							<view v-if="item.variant === 'badges'" class="yard-badges">
								<view v-for="(b, bi) in item.badges" :key="bi" class="yard-badge">
					...
					<scroll-view class="yard-gallery-scroll" scroll-x :show-scrollbar="false" :enable-flex="true">
						<view class="yard-gallery-row">
							<view v-for="(g, gi) in item.gallery" :key="gi" class="yard-g-item">
...
export default {
	components: { PawAvatar, PawVerifiedBadge, PawLikeIcon },
	data() {
		return {
			statusBarHeight: 20,
			menuRightWidth: 87,
