  ...
  <view class="yard-summary-card" :class="`yard-summary-card--${variant}`" @tap.stop="$emit('click', yard)">
    <view class="yard-summary-card__top">
      ...
      <view class="yard-summary-card__main">
        ...
        <YardLocationLine v-if="yard.location" :text="yard.location" />
      ...
      <text v-if="showDistance && yard.distance" class="yard-summary-card__distance">{{ yard.distance }}</text>
    </view>
...
<style scoped>
.yard-summary-card { padding: 14px 12px 15px; box-sizing: border-box; border-radius: 8px; background: #fff; overflow: hidden; }
.yard-summary-card--detail { padding: 14px 13px 20px; border-radius: 0; }
