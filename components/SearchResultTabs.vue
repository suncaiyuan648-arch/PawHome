<template>
	<view class="result-wrap">
		<PawTabs class="search-result-tabs" :model-value="modelValue" :items="tabs" variant="search"
			indicator-size="small" @update:model-value="$emit('update:modelValue', $event)"
		...
		<template v-if="modelValue === 'dynamic'">
			<scroll-view class="pane-scroll" scroll-y :show-scrollbar="false" :bounces="false" :enable-flex="true">
				<view class="dynamic-list">
					<view class="dynamic-column" v-for="(column, columnIndex) in dynamicColumns"
						:key="'dc-' + columnIndex">
						<view class="dynamic-card" :class="{ 'single-line-card': entry.item.title.length < 12 }"
							v-for="entry in column" :key="'d-' + entry.index">
							<view class="cover-wrap">
								...
								<view v-if="!entry.item.embeddedLocation" class="loc-pill">
									...
									<text class="loc-pill-text">{{ entry.item.distance }} {{ entry.item.district
										}}</text>
								</view>
							</view>
							...
							<view class="feed-foot">
								<view class="feed-user" @click.stop="openDynamicAuthor(entry.item)">
									<PawAvatar class="feed-avatar" :src="entry.item.avatar" fallback="/static/user.png"
										size="17px" />
									<text class="feed-name">{{ entry.item.name }}</text>
								</view>
								<view class="feed-like" @click.stop="toggleDynamicLike(entry.index)">
									<PawLikeIcon :liked="entry.item.liked" />
				...
				<view class="yard-list">
					<view class="yard-card" v-for="(yard, i) in yardList" :key="'y-' + i">
						<view class="yard-top" @click.stop="openYardCard(yard)">
							<PawAvatar class="yard-avatar" :src="yard.avatar" fallback="/static/user.png" :size="50"
								:clickable="true" @click.stop="openYardCard(yard)" />
							<view class="yard-mid">
			...
			<scroll-view class="pane-scroll" scroll-y :show-scrollbar="false" :bounces="false" :enable-flex="true">
				<view class="user-list">
					<view class="user-row" v-for="(u, i) in userList" :key="'u-' + i">
						<PawAvatar class="user-avatar" :src="u.avatar" fallback="/static/user.png" :size="42"
							:clickable="true" @click.stop="openUserRow(u)" />
