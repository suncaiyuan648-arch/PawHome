<template>
  <view class="feed-card" :class="{ 'feed-card--single': item.title && item.title.length < 12 }" @tap.stop="$emit('click', item)">
    <view class="feed-card__image-wrap"><image class="feed-card__image" :src="item.cover" mode="aspectFill" /><FeedCardLocation :distance="item.distance" :district="item.district" /></view>
...
export default { name: 'FeedCard', components: { PawAvatar, FeedCardLocation }, props: { item: { type: Object, default: () => ({}) } }, emits: ['click', 'user-click', 'like'] }
</script>
