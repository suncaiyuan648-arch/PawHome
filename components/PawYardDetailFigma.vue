<template>
  <view class="yard-detail-figma" :class="'state-' + activeState">
    ...
    <view class="yard-summary" @tap.stop="$emit('yard-click', yard)">
      <view class="yard-summary__info-row">
        ...
        <view class="yard-summary__details">
          <view class="yard-summary__identity">
            <view class="yard-summary__name-row"><text class="yard-summary__name">{{ yard.name }}</text><PawVerifiedBadge v-if="yard.verified !== false" /></view>
            <YardLocationLine :text="yard.location" />
          </view>
          <view class="yard-summary__metrics">
            <view class="yard-summary__tags"><text v-for="(tag, index) in yard.tags" :key="index" class="yard-summary__tag">{{ tag }}</text></view>
            <view class="yard-summary__stats">
              <view class="yard-summary__stat"><text class="yard-summary__stat-value">999斤</text><text class="yard-summary__stat-label">已获粮</text></view>
              <view class="yard-summary__stat"><text class="yard-summary__stat-value">13只</text><text class="yard-summary__stat-label">帮助领养</text></view>
              <view class="yard-summary__stat"><text class="yard-summary__stat-value">3天2小时</text><text class="yard-summary__stat-label">平均反馈时长</text></view>
            </view>
          </view>
        </view>
      ...
      <view class="yard-intro"><text>{{ introCopy }}</text><text v-if="!expandedIntro" class="more-word" @tap.stop="expandedIntro = true">... 全文</text><text v-else class="more-word" @tap.stop="expandedIntro = false"> 收起</text></view>
...
export default {
  ...
  components: { PawPageNav, PawAvatar, PawOwnerBadge, PawVerifiedBadge, YardLocationLine, LevelCapsule, FeedingSourceRow, YardFeedRankStrip, PawFixedActionBar, CommentThread, PawTabs },
